...
def Determinant1(_matrix):
    ...
    if order == 3:
        for i in range(3):
            ...
            print(f"{_matrix[0][i%3]*_matrix[1][(i+1)%3]*_matrix[2][(i+2)%3]}", end="")
            if i != 2:
                print(" + ", end="")
        for i in range(3):
            det -= _matrix[0][i%3]*_matrix[1][(i+2)%3]*_matrix[2][(i+1)%3]
            print(f" - {_matrix[0][i%3]*_matrix[1][(i+2)%3]*_matrix[2][(i+1)%3]}", end="")
        return det
    else:
        return "Ошибка"
